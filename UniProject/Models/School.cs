using System.ComponentModel;
using System.Runtime.CompilerServices;
using Xamarin.Forms;

namespace UniProject.Models
{
    // Class to store a single school's data from the database
    public class SchoolModel
    {
        public string SchoolName { get; set; }
        public string SchoolAddress { get; set; }
        public string ApplicationURL { get; set; }
        public string SchoolAcronym { get; set; }
        public int SemesterCost { get; set; }
        public int StudentsPerFaculty { get; set; }
        public double AverageGPA { get; set; }
        public int AverageSAT { get; set; }

        public SchoolModel() { }

        // Brandon: Credit to Josh for fixing our binding issue due to auto-property tomfoolery and my incomplete knowledge of C#
        public SchoolModel(string n, string a, string u, string c)
        {
            SchoolName = n;
            SchoolAddress = a;
            ApplicationURL = u;
            SchoolAcronym = c;
            SemesterCost = -1;
        }

        public SchoolModel(string n, string a, string u, string c, int t)
        {
            SchoolName = n;
            SchoolAddress = a;
            ApplicationURL = u;
            SchoolAcronym = c;
            SemesterCost = t;
        }

        // Auto-generated by Rider
        public SchoolModel(string schoolName, 
                            string schoolAddress, 
                            string applicationUrl, 
                            string schoolAcronym, 
                            int semesterCost, 
                            int studentsPerFaculty, 
                            double averageGpa, 
                            int averageSat)
        {
            SchoolName = schoolName;
            SchoolAddress = schoolAddress;
            ApplicationURL = applicationUrl;
            SchoolAcronym = schoolAcronym;
            SemesterCost = semesterCost;
            StudentsPerFaculty = studentsPerFaculty;
            AverageGPA = averageGpa;
            AverageSAT = averageSat;
        }
    }
}
